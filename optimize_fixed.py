...
• Plan: {account}
• File usage: {len(user_files.get(user_id, []))}/{limit}
...
"""
    ...
    blocks = [
        ...
        compact_table(
            ["Account", "Value"],
            [["Plan", account],
             ["Files", f"{len(user_files.get(user_id, []))}/{limit}"],
             ["User ID", str(user_id)]],
        ),
        bullet_list([
...
"""
    blocks = [
        ...
        compact_table(
            ["Metric", "Value"],
            [["Files", f"{len(user_files.get(user_id, []))}/{get_user_file_limit(user_id)}"],
...
📊 <b>Current Usage:</b> {current_files}/{limit}
...
"""
    blocks = [
        ...
        compact_table(
            ["Metric", "Value"],
            [["Current Usage", f"{current_files}/{limit}"],
...
"""
        ...
        if current_effective + len(runnable_paths) > limit:
            ...
            raise ValueError(
                ...
                f"{limit}-file limit. Please delete some files or upgrade "
                f"to premium."
            )
