...
class RichClient:
    ...
    async def _send_rich_via_placeholder(
        self,
        chat_id: int,
        ...
        *,
        ...
        """Send a rich card via placeholder + edit.

        Telegram only accepts ``rich_message`` on editMessageText in the
        deployed environment. So a minimal placeholder is sent first, then
        immediately edited with the rich blocks.
        """
        # FIX_RICH_SEND_v2: placeholder + edit approach.
        ...
        placeholder = await asyncio.to_thread(
            self._call_api, "sendMessage", placeholder_payload
        )
    ...
    async def _send_with_retry(
        ...
        *,
        ...
        Uses placeholder+edit because Telegram only accepts ``rich_message``
        on ``editMessageText`` in the deployed environment.
        """
