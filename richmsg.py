    ...
    async def _send_with_retry(
        ...
        *,
        ...
        for attempt in range(self.max_attempts):
            try:
                # NOTE: 'sendRichMessage' is used by the source project's rich_message.py.
                # If the Telegram endpoint changes in a future Bot API version, only this
                # method needs updating — the public RichClient API stays the same.
                response = await asyncio.to_thread(
                    self._call_api,
                    "sendRichMessage",
                    {"chat_id": chat_id, "rich_message": {"blocks": blocks}},
                )
